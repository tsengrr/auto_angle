...
def build_resnet50_unet(input_shape=(224, 224, 1)):
    """
    ResNet50 + U-Net
    - 灰階 → 3 通道用 Concatenate 複製
    - 不呼叫 ResNet 的 preprocess_input（避免 ImageNet mean/std 對灰階失真）
    - 輸入採 [-1, 1] 簡單正規化
    """
    inputs = layers.Input(shape=input_shape)
    x3 = layers.Concatenate()([inputs, inputs, inputs])   # 1ch → 3ch

    encoder = ResNet50(input_tensor=x3, include_top=False, weights='imagenet')

    # ResNet50 的 skip connections
    s1 = encoder.get_layer('conv1_relu').output          # 112x112
    s2 = encoder.get_layer('conv2_block3_out').output    # 56x56
    s3 = encoder.get_layer('conv3_block4_out').output    # 28x28
    s4 = encoder.get_layer('conv4_block6_out').output    # 14x14
    bridge = encoder.output                              # 7x7  (conv5_block3_out)
...
if __name__ == "__main__":
    ...
    MODEL_PATH = "/mnt/c/Users/chloe/OneDrive/桌面/auto_angle/vessel_lumen_resnet50_unet.h5"
    ...
    plt.title('Loss')
    plt.legend()
    plt.savefig("/mnt/c/Users/chloe/OneDrive/桌面/auto_angle/training_history_resnet50.png",
                bbox_inches='tight')
    plt.show()
